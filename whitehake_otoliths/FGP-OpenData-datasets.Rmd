---
title: "Creation of White Hake ageing datasets for OpenData"
...
## White Hake
White Hake otoliths are collected on the annual September RV survey and during scientific sampling of commercial fisheries.
...
```{r whitehake}
## RV data
y <- read.card(species=12, card.type="bio")
...
y.c <- read.card(year=1982, sampling="commercial", species=12, card.type="age") ## this loads all pre-1982 commercial ages
for(yy in c(1983:2017)){ ## loop over years
t.c <- read.card(year=yy, sampling="commercial", species=12, card.type="age")
y.c <- rbind(y.c,t.c)
}
...
hake.out.df <- rbind(rv.df, comm.df)
hake.out.df$latin.name_nom.latin <- "Merlucius bilinearis"
hake.out.df$english.name_nom.anglais <- "White Hake"
hake.out.df$french.name_nom.français <- "Merluche blanche"
...
o1 <- order(hake.out.df$source, hake.out.df$year, hake.out.df$month)

hake.out.fn <- "NAFO-4T-White-Hake-ages.csv"
write.csv(hake.out.df[o1,vars], file=hake.out.fn, row.names = FALSE)
