...
```{r witch, cache=TRUE}
## RV data
y <- read.card(species=41, card.type="bio")#rv$bio[which(rv$bio$species ==41),]
rv.df <- aggregate(fish.number~year+month, data=y[which(y$age.material==1),], length)
names(rv.df) <- c("year_année", "month_mois", "total.number_nombre.total")
rv.df$source <- "RV-4T"
...
## RV data
y <- read.card(species=42, card.type="bio")#rv$bio[which(rv$bio$species ==42),]
rv.df <- aggregate(fish.number~year+month, data=y[which(y$age.material==1),], length)
names(rv.df) <- c("year_année", "month_mois", "total.number_nombre.total")
rv.df$source <- "RV-4T"
...
Finally, let's write the full and partial dataframe to CSV


```{r final, include=TRUE, cache=TRUE}

out.fn <- "./output/NAFO-4T-age-estimate-structures-with-ages.csv"
write.csv(payload, file=out.fn, row.names = FALSE, fileEncoding = "UTF8")

# now without ages
out.fn <- "./output/NAFO-4T-age-estimate-structures.csv"
idx <- length(vars.bilingual)-1
new.cols <- vars.bilingual[0:idx]
