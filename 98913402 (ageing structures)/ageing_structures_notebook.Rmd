---
title: "Gulf Fisheries Centre Age Estimation Structures collection - All taxa in one
  go"
author: "Daniel Ricard, Laurie Maynard and David Fishman"
...
The [Age Estimation Structures collection held at the Gulf Fisheries Centre](https://dmapps/en/inventory/resources/98913402-688c-1615-9895-ec96b214be5a/) consists in *Teleost* saggital otoliths and *Chondrichthyan* vertebrae. 
For certain taxa, the physical specimens held in the collection are often accompanied with digital images. 
The individual details associated with each structure consists in capture date and location, length of individual and usually includes additional variables such as weight and maturity status.

The purpose of this notebook is to facilitate the generation and update of the FGP datasets associated with the Age Estimation Structures collection.

## Data dictionary
Generate the data dictionary.

```{r dictionary, echo=TRUE, include=FALSE}
#################################################################
...
vars.bilingual <- c(
  ...
  "year_année",
  "month_mois",
  "english.structure.type_type.de.structures.anglais",
  "french.structure.type_type.de.structures.français",
...
data.dic$description_fr <- c(
  "Source des structures d'estimation d'âges, peut soit provenir du relevé par navire de recherche dans la division 4T de l'OPANO (Research Vessel (RV-4T)), du relevé par navire de recherche dans les divisions 4RS de l'OPANO (Research Vessel (RV-4RS)), du relevé sentinelle de la division 4T de l'OPANO (SENT-4T),  d'échantillonnage des pêches commerciales (Commercial) ou d'observateurs en mer (Observateurs)",
  "Nom latin de l'espèce",
  "Nom anglais de l'espèce",
  "Nom français de l'espèce",
  "Année d'échantillonnage des structures d'estimation d'âges",
  "Mois d'échantillonnage des structures d'estimation d'âges",
  "Nom anglais du type de structures d'estimation d'âges",
  "Nom français du type de structures d'estimation d'âges",
  "Nombre total dans la collection",
  "Nombre qui ont été agées"
)

data.dic$description_en <- c(
  "Source of age estimation structures, can be either from the NAFO division 4T Research Vessel (RV-4T) survey, the NAFO divisions 4RS Research Vessel (RV-4RS) survey, the NAFO division 4T sentinel survey (SENT-4T), from commercial sampling (Commercial) or from at-sea observers (Observers)",
  "Latin name of the species",
  "English name of the species",
  "French name of the species",
  "Year when age estimation structures were collected",
  "Month when age estimation structures were collected",
  "English name of type of age estimation structure",
  "French name of type of age estimation structure",
  "Total number in collection",
  "Number that was aged"
)
data.dic.fn <- "./output/data-dictionary.csv"
# for the time being, we do not actually want the last row to be outputted.
write.csv(data.dic[0:8,], file = data.dic.fn, row.names = FALSE, fileEncoding = "UTF8")
...
## American Plaice
Otoliths from the September RV survey, from port sampling of commercial fisheries and from at-sea observers.

```{r plaice, cache=TRUE, include=FALSE}
## RV data
...
## observers data
y.o <- read.card(sampling = "obs", year = 2000:current_year, card = "age", species = 40)
...
out.df$english.structure.type_type.de.structures.anglais <- "otoliths"
out.df$french.structure.type_type.de.structures.français <- "otolithes"
...
## Atlantic Cod
Otoliths from the September RV survey, from sentinel programs, from port sampling of commercial fisheries and from at-sea observers.

Unresolved issues:
- observers data coming from read.card(sampling="obs") has erroneous dates for years 1994 to 2013 -> manually adding the year and setting month as "0" for now

```{r cod, cache=TRUE, include=FALSE}
## RV data
...
for(yy in c(1983:current_year)){ ## loop over years
...
## need to manually fix year for 1994 to 2013
## however, we also need to know the month to run aggregate
y.o.1 <- NULL
for(yy in c(1994:2013)){
  t.y.o <- read.card(sampling = "obs", year = yy, card = "age", species = 10)
  if(!is.null(t.y.o)){
    t.y.o$year<-yy
    t.y.o$month<-0 ##  a hack since there is no month available in the data files
    y.o.1 <- rbind(y.o.1,t.y.o)
  }
}

y.o <- read.card(sampling = "obs", year = 2014:current_year, card = "age", species = 10)
y.o <- rbind(y.o.1, y.o)
...
```{r hake, cache=TRUE, include=FALSE}
## RV data
...
## observers
y.o <- read.card(sampling = "obs", year = 2000:current_year, card = "age", species = 12)
obs.df <- aggregate(fish.number~year+month, data=y.o, length)
names(obs.df) <- c("year_année", "month_mois", "total.number_nombre.total")
obs.df$source <- "Observers"
...
```{r winter, cache=TRUE, include=FALSE}
## RV data
y <- read.card(species=43, card.type="bio")#rv$bio[which(rv$bio$species ==43),]
rv.df <- aggregate(fish.number~year+month, data=y[which(y$age.material==1),], length)
names(rv.df) <- c("year_année", "month_mois", "total.number_nombre.total")
rv.df$source <- "RV-4T"
...
y.c <- gulf::read.card(year=1982, sampling="commercial", species=43, card.type="age") ## this loads all pre-1982 commercial ages
for(yy in c(1983:current_year)){ ## loop over years
...
## observers data
y.o <- read.card(sampling = "obs", year = 2000:current_year, card = "age", species = 43)
obs.df <- aggregate(fish.number~year+month, data=y.o, length)
names(obs.df) <- c("year_année", "month_mois", "total.number_nombre.total")
obs.df$source <- "Observers"
...
## Yellowtail Flounder
Otoliths from the September RV survey, and from port sampling of commercial fisheries and from at-sea observers.

Remaining issues:
- are there really no commercial otoliths between 1982 and 2018?

```{r yellowtail, cache=TRUE, include=FALSE}
## RV data
...
## Witch Flounder
Otoliths from the September RV survey, from port sampling of commercial fisheries and from at-sea observers. Otoliths collected in the Quebec August survey in 4RS.

Unresolved issue:
- 2025 commercial samples should've been received from Jerry Lavers but are not in our systems

<!--
\Releves_Poissons_de_Fond_et_Crevette\Extractions_Oracle\RelevésRecherche
-->

```{r witch, cache=TRUE, include=FALSE}
## RV data
y <- read.card(species=41, card.type="bio")
rv.df <- aggregate(fish.number~year+month, data=y[which(y$age.material==1),], length)
names(rv.df) <- c("year_année", "month_mois", "total.number_nombre.total")
rv.df$source <- "RV-4T"
...
## observers data
y.o <- read.card(sampling = "obs", year = 1999:current_year, card = "age", species = 41)
obs.df <- aggregate(fish.number~year+month, data=y.o, length)
names(obs.df) <- c("year_année", "month_mois", "total.number_nombre.total")
obs.df$source <- "Observers"
...
out.4rs$english.structure.type_type.de.structures.anglais <- "otoliths"
out.4rs$french.structure.type_type.de.structures.français <- "otolithes"
...
all.out.df <- rbind(out.df, out.4rs[vars.bilingual])
...
payload <- rbind(payload, all.out.df[o1,vars.bilingual])


```

## Atlantic Herring
Otoliths from the September RV survey, from sentinel programs and from port sampling of commercial fisheries and from at-sea observers. 

Potential improvements: separate the source of the otoliths with a bit more nuance that "scientific surveys" and "other"


```{r herring, cache=TRUE, include=FALSE}
## herring otoliths and ages accessed through
## oracle.read.herring.bio
yrs <- 1974:current_year
y <- oracle.read.herring.bio(year=yrs)

## identify those coming from the RV survey
prince.idx <- grep("E.E. PRINCE.+", y$vessel.id)
hammond.idx <- grep("LADY HAMMOND.+", y$vessel.id)
needler.idx <- grep(".+NEEDLER.+", y$vessel.id)
templeman.idx <- grep(".+TEMPLEMAN.+", y$vessel.id)
teleost.idx <- grep("TELEOST", y$vessel.id)
cartier.idx <- grep("CARTIER", y$vessel.id)

rv.idx <- c(prince.idx, hammond.idx, needler.idx, templeman.idx, teleost.idx, cartier.idx)

y.rv <- y[rv.idx,]

## options for "source" for herring otoliths:
## RV-4T
## commercial
## observers

# plot(aggregate(fish.number~year, data=y, length), type="b"); grid()

rv.df <- aggregate(fish.number~year+month, data=y.rv, length) #[which(!is.na(y.rv$age)),]
names(rv.df) <- c("year_année", "month_mois", "total.number_nombre.total")
rv.df$source <- "Scientific surveys"

rv.df2 <- aggregate(fish.number~year+month, data=y.rv[which(!is.na(y.rv$age) & y.rv$age %in% c(0:20)),], length)
names(rv.df2) <- c("year_année", "month_mois", "number.aged_nombre.agées")
m.rv.df <- merge(rv.df, rv.df2, all.x=TRUE, by=c("year_année","month_mois"))
m.rv.df$source <- "Scientific surveys"
m.rv.df[is.na(m.rv.df$number.aged_nombre.agées),"number.aged_nombre.agées"] <- 0


## let's call all the other otoliths as coming from "other - autre"
not.rv.idx <- setdiff(1:nrow(y), rv.idx)

y.not.rv <- y[not.rv.idx,]
not.rv.df <- aggregate(fish.number~year+month, data=y.not.rv, length) #[which(!is.na(y.not.rv$age)),]
names(not.rv.df) <- c("year_année", "month_mois", "total.number_nombre.total")
not.rv.df$source <- "Other"

not.rv.df2 <- aggregate(fish.number~year+month, data=y.not.rv[which(!is.na(y.not.rv$age) & y.not.rv$age %in% c(0:20)),], length)
names(not.rv.df2) <- c("year_année", "month_mois", "number.aged_nombre.agées")
m.not.rv.df <- merge(not.rv.df, not.rv.df2, all.x=TRUE, by=c("year_année","month_mois"))
m.not.rv.df$source <- "Other"
m.not.rv.df[is.na(m.not.rv.df$number.aged_nombre.agées),"number.aged_nombre.agées"] <- 0

out.df <- rbind(m.rv.df, m.not.rv.df)
out.df$latin.name_nom.latin <- "Clupea harengus"
out.df$english.name_nom.anglais <- "Atlantic Herring"
out.df$french.name_nom.français <- "Hareng atlantique"
out.df$english.structure.type_type.de.structures.anglais <- "otoliths"
out.df$french.structure.type_type.de.structures.français <- "otolithes"
...
```{r skates, cache=TRUE, include=FALSE}
## RV data
y <- read.card(species=c(200:204), card.type="bio") 
...
rv.df$english.structure.type_type.de.structures.anglais <- "vertebrae"
rv.df$french.structure.type_type.de.structures.français <- "vertèbre"
...
Finally, let's write the full and partial data frames to CSV


```{r final, include=TRUE, cache=TRUE}
## replace month 0 with "unknown"
# idx <- which(payload$month_mois==13)
# payload[idx,"month_mois"] <- "unknown - inconnu"

out.fn <- "./output/NAFO-4T-age-estimation-structures-with-ages.csv"
write.csv(payload, file=out.fn, row.names = FALSE, fileEncoding = "UTF8")

# now without ages
out.fn <- "./output/NAFO-4T-age-estimation-structures.csv"
idx <- length(vars.bilingual)-1
new.cols <- vars.bilingual[0:idx]
write.csv(payload[new.cols], file=out.fn, row.names = FALSE, fileEncoding = "UTF8")

```

And for fun, make a few nice figures showing the number of otoliths over time and tabulate the number of otoliths in a summary table.

```{r plots1, echo=FALSE, fig.width=8, fig.height=4, out.width="100%"}
en.levels <- c("Atlantic Cod", "White Hake", "American Plaice", "Witch Flounder", "Yellowtail Flounder", "Winter Flounder", "Atlantic Herring", "Thorny Skate", "Smooth Skate", "Winter Skate")

payload$en.fac <- factor(payload$english.name_nom.anglais, levels=en.levels, ordered=TRUE)

fr.levels <- c("Morue franche", "Merluche blanche", "Plie canadienne", "Plie grise", "Limande à queue jaune", "Plie rouge", "Hareng atlantique", "Raie épineuse", "Raie lisse", "Raie tachetée")

payload$fr.fac <- factor(payload$french.name_nom.français, levels=fr.levels, ordered=TRUE)

library(ggplot2)
library(ggthemes)
g.en <- ggplot(data=payload, aes(x=year_année, y=total.number_nombre.total)) + 
  geom_col(aes(fill=en.fac)) + 
  theme_bw() + 
  labs(fill="Common name", x="Year", y="Number of otoliths collected") + #scale_fill_discrete()
  scale_fill_tableau()

g.en 

g.fr <- ggplot(data=payload, aes(x=year_année, y=total.number_nombre.total)) + 
  geom_col(aes(fill=fr.fac)) + 
  theme_bw() + 
  labs(fill="Nom commun", x="Année", y="Nombre d'otolithes prélevés") + 
  scale_fill_tableau() # scale_fill_discrete()
...
```{r plots2, echo=FALSE, fig.width=7, fig.height=7, out.width="100%"}

source.levels.en <- c("Commercial","RV-4T","RV-4RS","SENT-4T","Observers","Scientific surveys","Other")
source.levels.fr <- c("Commercial","RV-4T","RV-4RS","SENT-4T","Observateurs","Relevés scientifiques","Autre")

source.df <- data.frame(
  source.en=source.levels.en,
  source.fr=source.levels.fr
)

payload$source.en <- payload$source

payload$source.fr <- source.df[match(payload$source, source.df$source.en), "source.fr"] 

payload$source.en <- factor(payload$source.en, levels=source.levels.en, ordered=TRUE)
payload$source.fr <- factor(payload$source.fr, levels=source.levels.fr, ordered=TRUE)

g.en <- ggplot(data=payload, aes(x=year_année, y=total.number_nombre.total)) + 
  geom_col(aes(fill=en.fac)) + 
  facet_grid(rows=vars(source.en)) +
  theme_bw() + 
  labs(fill="Common name", x="Year", y="Number of otoliths collected") + 
  scale_fill_tableau() # scale_fill_discrete()
...
g.fr <- ggplot(data=payload, aes(x=year_année, y=total.number_nombre.total)) + 
  geom_col(aes(fill=fr.fac)) + 
  facet_grid(rows=vars(source.fr)) +
  theme_bw() + 
  labs(fill="Nom commun", x="Année", y="Nombre d'otolithes prélevés") + 
  scale_fill_tableau() # scale_fill_discrete()
...
agg1 <- aggregate(total.number_nombre.total~english.name_nom.anglais+year_année, data=payload, sum)
agg2 <- aggregate(number.aged_nombre.agées~english.name_nom.anglais+year_année, data=payload, sum)

en.levels <- c("Atlantic Cod", "White Hake", "American Plaice", "Witch Flounder", "Yellowtail Flounder", "Winter Flounder", "Atlantic Herring", "Thorny Skate", "Smooth Skate", "Winter Skate")

agg1$en.fac <- factor(agg1$english.name_nom.anglais, levels=en.levels, ordered=TRUE)


#my.df <- merge(agg1, agg2, by=c("english.name_nom.anglais","year_année"))
my.df <- agg1
oo <- order(my.df$en.fac, my.df$year_année)
my.df <- my.df[oo,]
...
en.levels, 
function(x){
  return(range(grep(x, my.df$en.fac)))
  }
)
)
t.i <- cbind(t.i, t.i[,2]-t.i[,1])
pr.args <- paste(
  paste0('"', en.levels, '"', '=', t.i[,3]),
collapse=','
      )

my.df$ri <- 1:nrow(my.df)

knitr::kable(my.df, row.names = FALSE) |> 
  column_spec(4, extra_css = "display: none;") |> 
