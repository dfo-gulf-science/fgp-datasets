...
data.dic$description_fr <- c("Source des otolithes, peut soit provenir du relevé par navire de recherche dans la division 4T de l'OPANO (Research Vessel (RV-4T)), du relevé par navire de recherche dans les divisions 4RS de l'OPANO (Research Vessel (RV-4RS))  ou d'échantillonnage des pêches commerciales (Commercial)", "Nom latin de l'espèce", "Nom anglais de l'espèce", "Nom français de l'espèce", "Année d'échantillonnage des otolithes", "Mois d'échantillonnage des otolithes", "Nombre d'otolithes dans la collection")

data.dic$description_en <- c("Source of otoliths, can be either from the NAFO 4T Research Vessel (RV-4T) survey, the NAFO divisions 4RS Research Vessel (RV-4RS) survey or from commercial sampling (Commercial)", "Latin name of the species", "English name of the species", "French name of the species", "Year when otoliths were collected", "Month when otolith were collected", "Number of otoliths in collection")
...
for(yy in c(1983:2002,2004:2019)){ ## loop over years
...
names(rv.df2) <- c("year_année", "month_mois", "number.otoliths.aged_nombre.otolithes.agés")
m.rv.df <- merge(rv.df, rv.df2, all.x=TRUE, by=c("year_année","month_mois"))
m.rv.df$source <- "RV"
m.rv.df[is.na(m.rv.df$number.otoliths.aged_nombre.otolithes.agés),"number.otoliths.aged_nombre.otolithes.agés"] <- 0
...
names(comm.df2) <- c("year_année", "month_mois", "number.otoliths.aged_nombre.otolithes.agés")
m.comm.df <- merge(comm.df, comm.df2, all.x=TRUE, by=c("year_année","month_mois"))
m.comm.df$source <- "Commercial"
m.comm.df[is.na(m.comm.df$number.otoliths.aged_nombre.otolithes.agés),"number.otoliths.aged_nombre.otolithes.agés"] <- 0
...
vars <- c("source", "latin.name_nom.latin", "english.name_nom.anglais", "french.name_nom.français", "year_année","month_mois","number.otoliths_nombre.otolithes","number.otoliths.aged_nombre.otolithes.agés")
o1 <- order(out.df$source, out.df$year_année, out.df$month_mois)
...
## Witch Flounder
Otoliths from the September RV survey, from other surveys, from sentinel programs and from commercial fisheries. Otoliths collected in the Quebec August survey in 4RS.
```{r witch, cache=TRUE}
## RV data
